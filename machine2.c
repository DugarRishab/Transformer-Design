...
int i, type;
float Et, K, KVA, m, Bm, Ki, f, phim, Ai, Agi, ct, d, p, q, r, s;
/*
Et = Voltage per turn
KVA = kVA ratings
...
void cd()
{
	...
	printf("\t\t\tCORE DESIGN\n");
	fprintf(fp, "\n******************************************************************\n");
	fprintf(fp, "\t\tCORE DESIGN OF TRANSFORMER\n");
	fprintf(fp, "******************************************************************\n\n\n\n\n");
	printf("Enter the kVA rating of the transformer(kVA):");
	scanf("%f", &KVA);
	fprintf(fp, "kVA rating of the transformer is %0.2f kVA", KVA);
	printf("\nK = 0.45 For Distribution Transformer,\nK = 0.6 to 0.7 For power Transformer \n");
	...
	phim = Et / (4.44 * f);
	fprintf(fp, "\nFlux in the core is %0.6f Wb", phim);
	...
	case 1:
		...
		break;
	case 2:
		fprintf(fp, "\nType of the core is Cruciform (2 Stepped)");
		ct = 0.56;
		d = sqrt(Ai / ct);
		fprintf(fp, "\nDiameter of the core is %0.4f m", d);
		p = 0.85 * d;
...
conductor:
	...
	if (cls < 14)
	{
		printf("\nThe calculation does not satisfy (14) minimum clearance. Recalculate.");
		fprintf(fp, "\nThe calculation does not satisfy (14) minimum clearance. Recalculate.");
		goto conductor;
	}
...
cal:
	...
	if (con == 1)
	{
		...
		scanf("%f", &dp1);
		fprintf(fp, "\nStandard value of the diameter(Ref Table 23.4)is %0.3f mm", dp1);
